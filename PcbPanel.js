...
export default function PcbPanel(props) { 
  ...
  const code = props.str.trim().toLowerCase().replace(/([^rgbmcyows])/g, "s")// + Array(ppos.length).fill('s').join('').substr(0, ppos.length);
  let polygons = [];
  for (let i = 0; i < code.length; i++) {
    ...
    for (let xy in base_polygon) {
      if (xy < 1) {
        points += ((base_polygon[0][0] + offset[0]) * 3 + "," + (base_polygon[0][1] + offset[1]) * 3) + " "
      } else {
        points += ((base_polygon[xy][0] + offset[0]) * 3 + "," + (base_polygon[xy][1] + offset[1]) * 3) + " "
      }
    }
    polygons.push([points,base_colors[code.substr(i, 1)]])
  }

  const polys = polygons.map((p, id)=>{return <Polygon points={p[0]} fill={p[1]} key={id}/>});
  const height = 150
  const width = 240
  return (
